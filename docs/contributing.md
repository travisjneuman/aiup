...
4. Never add telemetry or invoke `sudo` directly. Disclose when an official provider installer, such as Homebrew's initial setup, may request administrator authentication.
...
7. Switching an existing app to Homebrew must use `--adopt` first and must not pass `--zap` or directly delete `~/Library`; do not promise universal vendor-specific settings preservation.
